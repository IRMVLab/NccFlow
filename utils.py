...
This library contains the various util functions used in NccFlow.
"""
...
from NccFlow.resampler import resampler
...
def compute_loss(
  ...
  for key in warps:
    ...
    if 'smooth2' in weights or 'smooth1' in weights:
      ...
      if 'smooth1' in weights:
        ...
        if plot_dir is not None:
          plotting.plot_smoothness(key, images, weights_x, weights_y,
      ...
      if 'smooth2' in weights:
        ...
        if plot_dir is not None:
          plotting.plot_smoothness(key, images, weights_xx, weights_yy,
                                         robust_l1(flow_gxx),
    ...
    if 'intersect' in weights:
        intersect = intersect_loss(images[i], flows[key], tf.squeeze(mask_level0, axis=-1))
        losses['intersect'] += weights['intersect'] * intersect / num_pairs

    if 'flow_blocked' in weights:
        not_flow_blocked = not_flow_block_loss(images[i], flows[key], tf.squeeze(mask_level0, axis=-1))
        losses['flow_blocked'] += weights['flow_blocked'] * not_flow_blocked / num_pairs
    ...
    if 'selfsup' in weights:
      ...
      if plot_dir is not None:
        plotting.plot_selfsup(key, images, flows, teacher_flow,
...
def intersect_loss(image, flows, mask):
    a = image.get_shape()
    B, H, W, _ = a.as_list()
    flows = flows[0]
    flows = tf.cast(flows, dtype=tf.float32)
    da = tf.fill([1, H-2, W-2], 1e-8)
    loss = 0.
    zero = tf.fill([1, H-2, W-2], 0.)
    one = tf.fill([1, H - 2, W - 2], 1.)
    three = tf.fill([1, H - 2, W - 2], 3.)
    for i in range(9):
        if i == 4:
            continue
        C = i % 3
        D = i // 3
        k = tf.fill([1, H-2, W-2], C - 1)
        k = tf.cast(k, dtype=tf.float32)
        m = tf.fill([1, H-2, W-2], D - 1)
        m = tf.cast(m, dtype=tf.float32)
        rgb = tf.abs(image[0, 1:H-1, 1:W-1, 0]-image[0, C:H-2+C, D:W-2+D, 0]) + tf.abs(image[0, 1:H-1, 1:W-1, 1]-image[0, C:H-2+C, D:W-2+D, 1]) + tf.abs(image[0, 1:H-1, 1:W-1, 2]-image[0, C:H-2+C, D:W-2+D, 2])
        Wi = tf.exp(- tf.divide(rgb, three))
        deta = tf.add(-tf.multiply(flows[0, 1:H-1, 1:W-1, 0], flows[0, C:H-2+C, D:W-2+D, 1]), tf.multiply(flows[0, 1:H-1, 1:W-1, 1], flows[0, C:H-2+C, D:W-2+D, 0]))
        deta = tf.where(tf.equal(deta, 0.0), da, deta)
        lmd = tf.add(-tf.multiply(flows[0, C:H-2+C, D:W-2+D, 1], k), tf.multiply(flows[0, C:H-2+C, D:W-2+D, 0], m))
        lmd = tf.divide(lmd, deta)
        mu = tf.subtract(tf.multiply(flows[0, 1:H-1, 1:W-1, 0], m), tf.multiply(flows[0, 1:H-1, 1:W-1, 1], k))
        mu = tf.divide(mu, deta)
        Ei = abs_robust_loss(tf.exp(-tf.abs(lmd - 0.5) ** 2)) + abs_robust_loss(tf.exp(-tf.abs(mu - 0.5) ** 2))
        lmd_mask_1 = tf.where(tf.less(lmd, 1.0), one, zero)
        mu_mask_1 = tf.where(tf.less(mu, 1.0), one, zero)
        lmd_mask_2 = tf.where(tf.greater(lmd, 0.0), one, zero)
        mu_mask_2 = tf.where(tf.greater(mu, 0.0), one, zero)
        lmd_mu_mask = lmd_mask_1 * lmd_mask_2 * mu_mask_1 * mu_mask_2
        lmd_mu_mask = tf.stop_gradient(lmd_mu_mask)
        Ei = Ei * mask[0, C:H-2+C, D:W-2+D] * lmd_mu_mask * Wi
        Ei_sum = tf.reduce_sum(input_tensor=Ei)
        loss_mean = Ei_sum / (
            tf.reduce_sum(mask[0, C:H-2+C, D:W-2+D] * lmd_mu_mask + 1e-6))
        loss = loss_mean + loss

    loss = loss / 8.
    return loss

def not_flow_block_loss(image, flows, mask):
    loss = 0.0
    a = image.get_shape()
    B, H, W, _ = a.as_list()
    flows = flows[0]
    flows = tf.cast(flows, dtype=tf.float32)
    ones = {5, 6, 9, 10}
    one = tf.fill([1, H-3, W-3], 1.)
    zero = tf.fill([1, H-3, W-3], 0.)
    da = tf.fill([1, H - 3, W - 3], 1e-8)

    for i in range(16):
        C = i % 4
        D = i // 4
        K = tf.fill([1, H - 3, W - 3], C-1.)
        M = tf.fill([1, H - 3, W - 3], D-1.)
        if i in ones:
            continue
        else:
            #First: compute the p_mask
            #compute the used X coordinate
            Xba = (flows[0, 1:-2, 1:-2, 0] - flows[0, 2:-1, 1:-2, 0] - one)
            Xap = (flows[0, C:H-3+C, D:W-3+D, 0] - flows[0, 1:-2, 1:-2, 0] + K)
            Xda = (flows[0, 1:-2, 1:-2, 0] - flows[0, 1:-2, 2:-1, 0])
            Xca = (flows[0, 1:-2, 1:-2, 0] - flows[0, 2:-1, 2:-1, 0] - one)
            Xbd = Xba - Xda
            Xcb = Xca - Xba
            Xdc = Xda - Xca
            Xbp = Xap + Xba
            ...
            #compute the uesd Y coordinate
            Yba = (flows[0, 1:-2, 1:-2, 1] - flows[0, 2:-1, 1:-2, 1])
            Yap = (flows[0, C:H-3+C, D:W-3+D, 1] - flows[0, 1:-2, 1:-2, 1] + M)
            Yda = (flows[0, 1:-2, 1:-2, 1] - flows[0, 1:-2, 2:-1, 1] - one)
            Yca = (flows[0, 1:-2, 1:-2, 1] - flows[0, 2:-1, 2:-1, 1] - one)
            Ybd = Yba - Yda
            Ycb = Yca - Yba
            Ydc = Yda - Yca
            Ybp = Yap + Yba
            ...
            #compute the ABC
            abc1 = tf.multiply(Xba, Ybp) - tf.multiply(Xbp, Yba)
            abc2 = tf.multiply(-Xca, Yap) - tf.multiply(Xap, -Yca)
            abc3 = tf.multiply(Xcb, Ycp) - tf.multiply(Xcp, Ycb)
            abc1_mask = tf.where(tf.greater_equal(abc1, 0.0), one, zero)
            abc2_mask = tf.where(tf.greater_equal(abc2, 0.0), one, zero)
            abc3_mask = tf.where(tf.greater_equal(abc3, 0.0), one, zero)
            abc_mask = abc1_mask * abc2_mask * abc3_mask


            #compute the ACD
            acd1 = tf.multiply(Xca, Ycp) - tf.multiply(Xcp, Yca)
            acd2 = tf.multiply(-Xda, Yap) - tf.multiply(Xap, -Yda)
            acd3 = tf.multiply(Xdc, Ydp) - tf.multiply(Xdp, Ydc)
            acd1_mask = tf.where(tf.greater_equal(acd1, 0.0), one, zero)
            acd2_mask = tf.where(tf.greater_equal(acd2, 0.0), one, zero)
            acd3_mask = tf.where(tf.greater_equal(acd3, 0.0), one, zero)
            acd_mask = acd1_mask * acd2_mask * acd3_mask


            # compute the ABD
            abd1 = tf.multiply(Xba, Ybp) - tf.multiply(Xbp, Yba)
            abd2 = tf.multiply(-Xda, Yap) - tf.multiply(Xap, -Yda)
            abd3 = tf.multiply(-Xbd, Ydp) - tf.multiply(Xdp, -Ybd)
            abd1_mask = tf.where(tf.greater_equal(abd1, 0.0), one, zero)
            abd2_mask = tf.where(tf.greater_equal(abd2, 0.0), one, zero)
            abd3_mask = tf.where(tf.greater_equal(abd3, 0.0), one, zero)
            abd_mask = abd1_mask * abd2_mask * abd3_mask


            # compute the BCD
            bcd1 = tf.multiply(Xbd, Ybp) - tf.multiply(Xbp, Ybd)
            bcd2 = tf.multiply(Xdc, Ydp) - tf.multiply(Xdp, Ydc)
            bcd3 = tf.multiply(Xcb, Ycp) - tf.multiply(Xcp, Ycb)
            bcd1_mask = tf.where(tf.greater_equal(bcd1, 0.0), one, zero)
            bcd2_mask = tf.where(tf.greater_equal(bcd2, 0.0), one, zero)
            bcd3_mask = tf.where(tf.greater_equal(bcd3, 0.0), one, zero)
            bcd_mask = bcd1_mask * bcd2_mask * bcd3_mask


            # compute the fianl mask as P_mask
            p_mask = (abc_mask + acd_mask) * (abd_mask + bcd_mask)
            p_mask = tf.where(tf.greater(p_mask, 0.0), one, zero)
            p_mask = tf.stop_gradient(p_mask)


            # Second: compute the loss
            d_ab = tf.abs(Yba * Xap - Xba * Yap) / (tf.abs(tf.sqrt(Yba * Yba + Xba * Xba)) + da)
            d_bc = tf.abs(Ycb * Xbp - Xcb * Ybp) / (tf.abs(tf.sqrt(Ycb * Ycb + Xcb * Xcb)) + da)
            d_cd = tf.abs(Ydc * Xcp - Xdc * Ycp) / (tf.abs(tf.sqrt(Ydc * Ydc + Xdc * Xdc)) + da)
            d_da = tf.abs(Yda * Xdp - Xda * Ydp) / (tf.abs(tf.sqrt(Yda * Yda + Xda * Xda)) + da)


            #choose the min d as the fianl d
            d = tf.where(tf.greater(d_ab, d_bc), d_bc, d_ab)
            d = tf.where(tf.greater(d_cd, d), d, d_cd)
            d = tf.where(tf.greater(d_da, d), d, d_da)
            d = tf.where(tf.equal(d, 0.0), da, d)

            #compute the loss
            Ei = tf.exp(-1 / d)
            Ei = abs_robust_loss(Ei)
            Ei = Ei * p_mask * mask[0, C:H - 3 + C, D:W - 3 + D]
            Ei_sum = tf.reduce_sum(input_tensor=Ei)
            num = tf.reduce_sum(p_mask * mask[0, C:H-3+C, D:W-3+D])
            if num == 0:
                loss_mean = 0.
            else:
                loss_mean = Ei_sum / num
            loss = loss_mean + loss
