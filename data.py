...
from NccFlow.nccflow_data import generic_flow_dataset
from NccFlow.nccflow_data import kitti
from NccFlow.nccflow_data import sintel
...
def make_eval_function(eval_on, height, width, progress_bar, plot_dir,
                       num_plots):
  """Build an evaluation function for nccflow.
  ...
  for format_and_path in eval_on.split(';'):
    ...
    if 'kitti' in data_format:
      ...
      dataset = generic_flow_dataset.make_dataset(path, mode='eval')
      eval_fn = partial(
          generic_flow_dataset.evaluate,
          prefix=data_format,
          max_num_evals=1000,  # We do this to avoid evaluating on 22k samples.
          has_occlusion=False)
      eval_keys += generic_flow_dataset.list_eval_keys(prefix=data_format)
    elif 'sintel' in data_format:
      if 'benchmark' in data_format:
        # pylint:disable=g-long-lambda
        # pylint:disable=cell-var-from-loop
        eval_fn = lambda nccflow: sintel.benchmark(inference_fn=nccflow.infer,
  ...
  def eval_function(nccflow):
    result = dict()
    for eval_fn, ds in eval_functions_and_datasets:
      results = eval_fn(
          nccflow.infer, ds, height,
