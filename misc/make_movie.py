...
python3 nccflow.make_movie.py -c opt -- \
--alsologtostderr \
...
def main(unused_argv):
  ...
  nccflow = evaluator.build_network(batch_size=1)
  ...
  for i, (batch, _) in enumerate(train_it):
    ...
    sys.stdout.write(':')
    sys.stdout.flush()
    flow = nccflow.infer(batch[0, 0], batch[0, 1], input_height=FLAGS.height,
                       input_width=FLAGS.width)
    plotting.plot_movie_frame(FLAGS.plot_dir, i, batch[0, 0].numpy(),
                                    flow.numpy())
