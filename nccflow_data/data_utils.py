...
def parse_data(proto,
  ...
  if height is not None and width is not None:
    images = utils.resize(images, height, width, is_flow=False)
  output = [images]
  ...
  if include_flow:
    ...
    if height is not None and width is not None and resize_gt_flow:
      flow_uv = utils.resize(flow_uv, height, width, is_flow=True)
    else:
      if gt_flow_shape is not None:
        flow_uv.set_shape(gt_flow_shape)
    # To be consistent with nccflow internals, we flip the ordering of flow.
...
def evaluate(
  ...
  Args:
    inference_fn: An inference function that produces a flow_field from two
      images, e.g. the infer method of NccFlow.
  ...
  for test_batch in it:
    ...
    inference_time_in_ms, (flow, soft_occlusion_mask) = utils.time_it(
