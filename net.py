...
"""NccFlow: Unsupervised Optical Flow.
...
from NccFlow import model
from NccFlow import utils
...
class NccFlow(object):
  ...
  def __init__(
    ...
    self._feature_model = model.PWCFeaturePyramid(
    ...
    self._flow_model = model.PWCFlow(
  ...
  def restore(self, reset_optimizer=False, reset_global_step=False):
    ...
    try:
      ...
      print('Error while attempting to restore NccFlow models:', e)
    if reset_optimizer:
      self._make_or_reset_optimizer()
      self._make_or_reset_checkpoint()
  ...
  def batch_infer_no_tf_function(self,
    ...
    if resize_flow_to_img_res:
      flow = utils.resize(flow, orig_height, orig_width, is_flow=True)
  ...
  def infer_occlusion(self, flow_forward, flow_backward):
    ...
    _, _, _, occlusion_masks, _, _ = utils.compute_warps_and_occlusion(
        flows,
  ...
  def train(self,
    ...
    for num, batch in zip(range(num_steps), data_it):
  ...
  def compute_loss(self,
    ...
    if self._train_with_supervision:
      ...
      flows = utils.compute_flow_for_supervised_loss(
          self._feature_model, self._flow_model, batch=batch, training=True)
      losses = utils.supervised_loss(weights, ground_truth_flow,
    ...
    flows, selfsup_transform_fns = utils.compute_features_and_flow(
        self._feature_model,
        self._flow_model,
        batch=batch,
    ...
    warps, valid_warp_masks, _, not_occluded_masks, fb_sq_diff, fb_sum_sq = utils.compute_warps_and_occlusion(
        flows,
    ...
    warped_images = utils.apply_warps_stop_grad(images, warps, level=0)
